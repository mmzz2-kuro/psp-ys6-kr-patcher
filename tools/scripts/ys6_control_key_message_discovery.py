#!/usr/bin/env python3
"""Extract and locate Ys VI v140/v141 and v150-v157 message images."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(
        "tools/patchdata/ys6_additional_images/source_images/control_key_messages"))
    ...
    with args.iso.open("rb") as handle:
        for stem in STEMS:
            ...
            source_containers[f"{stem}.dds.z"] = container
            sources[stem] = {
                "iso_path": iso_path,
        ...
        for raw_path, record in iter_files(handle, root):
            ...
            for entry in parse_archive(archive):
                source_container = source_containers.get(entry.name.casefold())
                if source_container is None or entry.flags not in DATA_FLAGS or entry.size <= 0:
                    continue
                ...
                copies[stem].append({
                    ...
                    "exact_container_match": data == source_container,
                    "exact_payload_match": bool(
                        valid and payload is not None and sha256(payload) == source["payload_sha256"]),
                })

    sheet = Image.new("RGBA", (1280, 176), (28, 28, 28, 255))
    draw = ImageDraw.Draw(sheet)
    for position, (stem, image) in enumerate(previews):
        left = position % 5 * 256
        top = position // 5 * 88
        sheet.alpha_composite(image, (left, top + 20))
        draw.text((left + 5, top + 3), f"{stem} 256x64", fill=(255, 255, 255, 255))
