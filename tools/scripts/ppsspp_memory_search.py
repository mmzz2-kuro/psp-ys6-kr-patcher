...
def search(pid:int,patterns:list[tuple[str,bytes]])->dict:
 handle=kernel32.OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ,False,pid)
 if not handle:raise OSError(ctypes.get_last_error(),"OpenProcess failed")
 hits={name:[] for name,_ in patterns};regions=0;read_bytes=0;address=0;mbi=MEMORY_BASIC_INFORMATION()
 try:
  ...
  return {"pid":pid,"regions_read":regions,"bytes_read":read_bytes,"patterns":[{"name":name,"size":len(data),"hits":hits[name]} for name,data in patterns]}
...
def main(argv=None):
 ...
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("pid",type=int);p.add_argument("--pattern",action="append",default=[],help="NAME=FILE");p.add_argument("--hex-pattern",action="append",default=[],help="NAME=HEX");p.add_argument("--output",type=Path);a=p.parse_args(argv)
 patterns=[]
 ...
 if not patterns:raise ValueError("at least one pattern is required")
 result=search(a.pid,patterns);rendered=json.dumps(result,ensure_ascii=False,indent=2)+"\n"
