...
def execute(args)->dict:
 ...
 ending_workspace=getattr(args,"ending_movie_workspace",None)
 if ending_workspace:
  ending_manifest=json.loads((ending_workspace/"manifest.json").read_text(encoding="utf-8-sig"))
  for movie in ending_manifest["movies"]:
   iso_path=movie["iso_path"];asset=ending_workspace/movie["file"]
   original_movie,record=read_iso_file(iso,iso_path)
   if len(original_movie)!=int(movie["original_size"]) or sha256(original_movie)!=movie["original_sha256"]:
    raise IntegratedBuildError(f"ending movie original mismatch: {iso_path}")
   replacement=asset.read_bytes()
   if sha256(replacement)!=movie["output_sha256"]:
    raise IntegratedBuildError(f"ending movie asset hash mismatch: {movie['file']}")
   allocated=((record.data_length+SECTOR_SIZE-1)//SECTOR_SIZE)*SECTOR_SIZE
   if len(replacement)>allocated:
    raise IntegratedBuildError(f"ending movie allocation overflow: {movie['file']}: {len(replacement)} > {allocated}")
   pmf_report=inspect_pmf(asset)
   if pmf_report["frame_count"]!=int(movie["frame_count"]) or pmf_report["size"]!=len(replacement):
    raise IntegratedBuildError(f"ending movie structure mismatch: {movie['file']}")
   ending_movie_reports.append({"id":movie["id"],"iso_path":iso_path,"file":movie["file"],"original_size":len(original_movie),"replacement_size":len(replacement),"allocated_size":allocated,"remaining_slack":allocated-len(replacement),"original_sha256":sha256(original_movie),"output_sha256":sha256(replacement),"frame_count":pmf_report["frame_count"],"video_pes_count":pmf_report["video_pes_count"]})
   extra_replacements.append(Replacement(iso_path,asset,len(original_movie),sha256(original_movie)))
 xmb_workspace=getattr(args,"xmb_workspace",None)
 if xmb_workspace:
  xmb_manifest=json.loads((xmb_workspace/"manifest.json").read_text(encoding="utf-8-sig"))
  for item in xmb_manifest["assets"]:
   iso_path=item["iso_path"];asset=xmb_workspace/item["file"]
   original,record=read_iso_file(iso,iso_path);replacement=asset.read_bytes()
   if len(original)!=int(item["original_size"]) or sha256(original)!=item["original_sha256"]:raise IntegratedBuildError(f"XMB image original mismatch: {iso_path}")
   if len(replacement)!=int(item["output_size"]) or sha256(replacement)!=item["output_sha256"]:raise IntegratedBuildError(f"XMB image asset mismatch: {item['file']}")
   try:
    with Image.open(asset) as image:image.load();image_size=image.size;image_mode=image.mode
   except Exception as exc:raise IntegratedBuildError(f"XMB image PNG invalid: {item['file']}: {exc}") from exc
   if image_size!=(int(item["width"]),int(item["height"])) or image_mode!=item["mode"]:raise IntegratedBuildError(f"XMB image format mismatch: {item['file']}: {image_size} {image_mode}")
   allocated=((record.data_length+SECTOR_SIZE-1)//SECTOR_SIZE)*SECTOR_SIZE
   if len(replacement)>allocated:raise IntegratedBuildError(f"XMB image allocation overflow: {item['file']}: {len(replacement)} > {allocated}")
   xmb_image_reports.append({"id":item["id"],"iso_path":iso_path,"file":item["file"],"width":image_size[0],"height":image_size[1],"mode":image_mode,"original_size":len(original),"replacement_size":len(replacement),"allocated_size":allocated,"remaining_slack":allocated-len(replacement),"original_sha256":sha256(original),"output_sha256":sha256(replacement)})
   extra_replacements.append(Replacement(iso_path,asset,len(original),sha256(original)))
