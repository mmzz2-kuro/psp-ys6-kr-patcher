...
class PatchBuildEditor(ttk.Frame):
    def __init__(self, parent, app: DialogueViewer) -> None:
        ...
        self.apply_analog_stick = tk.BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        self.ending_movies_check = ttk.Checkbutton(
        ...
        self.analog_stick_check = ttk.Checkbutton(
            image_options, text="아날로그 스틱 적용", variable=self.apply_analog_stick,
            command=lambda: self.refresh_data(silent=True))
        self.analog_stick_check.pack(side=tk.LEFT, padx=(12, 0))
    ...
    def start(self, mode: str) -> None:
        ...
        threading.Thread(
            target=self._worker,
            args=(mode, iso, output, font, overwrite, include_option_images, include_additional_images, include_ending_movies, include_xmb_image, include_analog_stick),
            daemon=True,
        ).start()

    def _worker(self, mode, iso, output, font, overwrite,
                include_option_images, include_additional_images, include_ending_movies, include_xmb_image,
                include_analog_stick) -> None:
        try:
            result = run_build(
